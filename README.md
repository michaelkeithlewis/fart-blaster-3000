...
- **MIDI TRIGGER** — Every MIDI Note On fires a fart instantly (samples are silence-trimmed on load, so the attack lands right on the beat). Velocity controls loudness. Playing a note chokes any random-timer farts and pushes the random engine back a full interval — the keyboard always takes the stage, while notes you play can still stack into chords. Works with your DAW's MIDI routing, a hardware controller, or the on-screen keyboard built into the plugin window. Set HOW MUCH? to OFF for pure keyboard-controlled flatulence.
